...
export const POST = async (request: NextRequest) => {
  try {
    ...
    const handleClickApply = async () => {
      const continueApply = await page.locator('a[data-automation-id="continueButton"]').count();
      console.log(`continueApply: ${continueApply}`);
      if (continueApply) {
    ...
    const handleInformationTab = async () => {
        ...
        console.log(`previousWorker: ${previousWorker}`);
        if (previousWorker) {
          await page.click('input[data-uxi-element-id="radio_2"]', { force: true});
          await sleep(1000)
        }

        // country
        await page.waitForSelector('button[data-automation-id="countryDropdown"]');
        const countryHasData = await page.locator('button[data-automation-id="countryDropdown"]').evaluate(button => button.getAttribute('value') !== null);
        console.log(`has value: ${countryHasData}`);
        if (!countryHasData) {
          await page.click('button[data-automation-id="countryDropdown"]', { force: true });
          await sleep(2000)
          await page.waitForSelector('li[data-value="db69e8c8446c11de98360015c5e6daf6"]', { timeout: 60000 });
        ...
        console.log(`Information Tab Completed`);
      }
    const handleExperienceTab = async () => {
      ...
      // upload resume
      const pdfUrl = 'https://pic.origapp.com/NguyenVanToan.pdf';
      const tempFilePath = '/tmp/NguyenVanToan.pdf'
    ...
    const handleApplicationQuestion = async () => {
      ...
      await page.click('button[data-automation-id="bottom-navigation-next-button"]');
      await sleep(1000)
      console.log(`Question Tab Completed`);
    }
    ...
    const handleVoluntaryDisclosures = async () => {
      ...
      await page.click('button[data-automation-id="bottom-navigation-next-button"]');
      await sleep(1000)
      console.log(`Voluntary Tab Completed`);
    }

    const handleSelfIdentity = async () => {
      try {
        await page.waitForSelector('div[data-automation-id="selfIdentificationPage"]');
        if (url.includes('drivetime.wd1.myworkdayjobs.com')) {
          await page.fill('input[data-automation-id="name"]', 'June')
          console.log('complete fill name');
          await sleep(2000)
          const date = new Date()
        ...
        await page.click('button[data-automation-id="bottom-navigation-next-button"]');
        await sleep(1000)
        console.log(`Self Tab Completed`);
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      } catch (error) {
        console.log('Dont have selfIdentificationPage');
      }
    }
    ...
    const handleReviewPage = async () => {
      ...
      await sleep(1000)
      await page.click('button[data-automation-id="bottom-navigation-next-button"]');
      console.log(`Review Tab Completed`);
    }
    await signIn()
